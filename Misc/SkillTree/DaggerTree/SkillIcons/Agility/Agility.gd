...
func _on_SkillButton_mouse_entered():
	...
	if PlayerSkills.Agility == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PlayerSkills.Agility == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
...
func _on_SkillButton_pressed():
	if PGS.skillpoints > 0:
		...
		if PGS.skill_selector == 0 and PlayerSkills.Agility == false:
