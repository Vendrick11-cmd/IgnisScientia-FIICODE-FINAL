...
func _on_SkillButton_mouse_entered():
	...
	if PGS.HealthPick == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.HealthPick == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.HealthPick == true:
...
func _on_SkillButton_pressed():
	if PGS.skillpoints > 0:
		if PGS.DreadedHeal == true:
			...
			if PGS.skill_selector == 0 and PGS.HealthPick == false:
