...
func _on_SkillButton_mouse_entered():
	...
	if PGS.Toughness == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.Toughness == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.Toughness == true:
...
func _on_SkillButton_pressed():
	if PGS.skillpoints > 0:
		if PGS.Bandage == true:
			...
			if PGS.skill_selector == 0 and PGS.Toughness == false:
				$SkillButton/Skill/AnimationPlayer.play("Click")
				PGS.skill_selector = 1

func _on_YesButton_pressed():
	PlayerStats.max_armor = 4
	$SkillButton/Skill/AnimationPlayer.play("Active")
