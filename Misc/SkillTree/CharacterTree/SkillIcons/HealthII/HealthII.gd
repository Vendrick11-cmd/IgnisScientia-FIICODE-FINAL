...
func _on_SkillButton_mouse_entered():
	...
	if PGS.HealthII == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.HealthII == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.HealthII == true:
...
func _on_SkillButton_pressed():
	if PGS.skillpoints > 0:
		if PGS.Toughness == true:
			...
			if PGS.skill_selector == 0 and PGS.HealthII == false:
