...
func _on_SkillButton_mouse_entered():
	...
	if PGS.FamRaven == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")
		
func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.FamRaven == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.FamRaven == true:
		$SkillButton/Skill/AnimationPlayer.play("Active")
		
func _on_SkillButton_pressed():
	if PGS.has_found_pet == true and PGS.FamCat == false and PGS.FamWolf == false and PGS.FamParrot == false:
		PGS.active_skill = true
		if PGS.skill_selector == 0 and PGS.FamRaven == false:
