...
func _on_SkillButton_mouse_entered():
	...
	if PGS.HealthIII == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.HealthIII == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.HealthIII == true:
		$SkillButton/Skill/AnimationPlayer.play("Active")

func _on_SkillButton_pressed():
	if PGS.VileBlood == true:
		PGS.active_skill = true
		if PGS.skill_selector == 0 and PGS.HealthIII == false:
