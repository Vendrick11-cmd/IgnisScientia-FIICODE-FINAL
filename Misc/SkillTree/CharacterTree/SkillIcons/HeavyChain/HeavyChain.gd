...
func _on_SkillButton_mouse_entered():
	...
	if PGS.HeavyChain == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")
		
func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.HeavyChain == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("Idle")
	if PGS.HeavyChain == true:
		$SkillButton/Skill/AnimationPlayer.play("Active")
		
func _on_SkillButton_pressed():
	if PGS.Dash == true:
		PGS.active_skill = true
		if PGS.skill_selector == 0 and PGS.HeavyChain == false:
