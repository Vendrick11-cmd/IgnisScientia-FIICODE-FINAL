...
func _on_SkillButton_mouse_entered():
	...
	if PGS.sight == true and PGS.active_skill == false:
		$SkillButton/Skill/AnimationPlayer.play("ActiveHover")

func _on_SkillButton_mouse_exited():
	if !$SkillButton/Skill/AnimationPlayer.current_animation == "Click" and PGS.sight == false and $Hover/YesNo/Activate.visible == false and PGS.active_skill == false:
